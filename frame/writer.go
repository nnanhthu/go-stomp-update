...
func NewFrameWriter(wsConn *websocket.Conn) *FrameWriter {
	return &FrameWriter{
		wsConn: wsConn,
	}
}
...
func (w *FrameWriter) Write(f *Frame) error {
	...
	if f == nil {
		// nil frame means send a heart-beat LF
		_, err = sb.Write(newlineSlice)
		if err != nil {
			return err
		}
	} else {
		_, err = sb.Write([]byte(f.Command))
		if err != nil {
			return err
		}
		_, err = sb.Write(newlineSlice)
		...
		if f.Header != nil {
			for i := 0; i < f.Header.Len(); i++ {
				key, value := f.Header.GetAt(i)
				//println("   ", key, ":", value)
				_, err = sb.Write(encodeValue(key))
				if err != nil {
					return err
				}
				_, err = sb.Write(colonSlice)
				if err != nil {
					return err
				}
				_, err = sb.Write(encodeValue(value))
				if err != nil {
					return err
				}
				_, err = sb.Write(newlineSlice)
	...
	var messages [1]string
	messages[0] = message

	frame, err := json.Marshal(messages)

	w.wsConn.WriteMessage(websocket.TextMessage, []byte(frame))
	return nil
}
